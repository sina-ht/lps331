/* -*- c-basic-offset: 8 -*-
 * lps331.c -- LPS331 Pressure Sensor Driver
...
static int update_rrd(char *rrdfilename, double pressure) {
	char buf[256];

	snprintf(buf, 256, "rrdtool update %s N:%g", rrdfilename, pressure);
	return system(buf);
}
...
int main(int argc, char **argv)
{
	...
	while (1) {
		if ((res = read_pressure(fd, &pressure)) < 0) {
#if 0
			fprintf(stderr, "read_pressure() failed: %s\n", strerror(errno));
			close(fd);
			return res;
#endif
			errcount++;
			...
			errcount = 0;
			if ((res = update_rrd(argv[1], pressure)) < 0) {
				fprintf(stderr, "rrdtool failed: %d\n", res);
				break;
			}
		}
		sleep(300);
